...
class CanvasWidget(tk.Frame):
    def __init__(self, parent, state, history, tool_registry, on_cursor_move):
        ...
        # PIL image sebagai backing store
        ...
        # Handle drag resize
        self._resize_handle = None  # 'right','bottom','corner'
    ...
    # ── PIL image access (diperlukan FillTool & EyedropperTool) ─────
    ...
    # ── Koordinat canvas piksel (setelah zoom & offset) ──────────────
    ...
    def _draw_all(self):
        ...
        # Gambar canvas putih (backing PIL image)
        ...
        # Render semua objek di atas PIL image
    ...
    def _render_object(self, obj, ox, oy, selected=False):
        ...
        if otype in ("line", "pencil"):
            ...
            if len(pts) >= 2:
                ...
                self._canvas.create_line(flat, fill=color, width=width,
    ...
    def _on_drag(self, event):
        if self._resize_handle and self._drag_start:
            ...
            new_img.paste(self._pil_image,
                          (0, 0, min(self._pil_image.width, self.state.width),
                           min(self._pil_image.height, self.state.height)))
